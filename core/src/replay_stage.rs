...
impl ReplayStage {
    ...
    fn handle_votable_bank(
        ...
        // Update tower configuration before voting
        tower.update_config();
    ...
    fn cache_tower_stats(
        ...
        stats.vote_threshold =
            tower.check_vote_stake_thresholds(slot, &stats.voted_stakes, stats.total_stake, true);
    ...
    fn mark_slots_duplicate_confirmed(
        ...
        mostly_confirmed_slots: &[(Slot, Hash)],
        blockstore: &Blockstore,
        bank_forks: &RwLock<BankForks>,
        progress: &mut ProgressMap,
        duplicate_slots_tracker: &mut DuplicateSlotsTracker,
        ...
        // Process mostly confirmed slots
        for (slot, frozen_hash) in mostly_confirmed_slots.iter() {
            if *slot <= root_slot {
                continue;
            }
            
            if let Some(prog) = progress.get_mut(slot) {
                if prog.fork_stats.mostly_confirmed_hash.is_none() {
                    prog.fork_stats.mostly_confirmed_hash = Some(*frozen_hash);
                    debug!("Marking slot {} as mostly confirmed with hash {}", slot, frozen_hash);
                }
            }
        }
    }
    ...
    fn tower_duplicate_confirmed_forks(
        ...
        for (slot, prog) in progress.iter() {
            ...
            } else if tower.is_slot_mostly_confirmed(*slot, voted_stakes, total_stake) {
                mostly_confirmed_forks.push((*slot, bank.hash()));
                debug!(
                    "validator fork mostly confirmed {} {}ms {:?}",
                    *slot,
                    duration,
...
pub(crate) mod tests {
    ...
    fn test_compute_bank_stats_confirmed() {
        ...
        // bank 1, so no slot should be confirmed.
        {
            ...
            let (confirmed_forks, mostly_confirmed_forks) = ReplayStage::tower_duplicate_confirmed_forks(
                &tower,
            ...
            assert!(mostly_confirmed_forks.is_empty());
        }
