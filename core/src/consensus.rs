...
impl Tower {
    ...
    pub fn new_with_config(
        ...
        bank: &Bank,
        mostly_confirmed_threshold: Option<f64>,
        threshold_ahead_count: Option<usize>,
        after_skip_threshold: Option<f64>,
        threshold_escape_count: Option<usize>,
    ) -> Self {
        let mut tower = Tower {
            node_pubkey: *node_pubkey,
    ...
    pub(crate) fn is_slot_mostly_confirmed(
        &self,
        slot: Slot,
        voted_stakes: &VotedStakes,
        total_stake: Stake,
    ) -> bool {
        let mostly_threshold = self.mostly_confirmed_threshold.unwrap_or(0.51);
        voted_stakes
            .get(&slot)
            .map(|stake| (*stake as f64 / total_stake as f64) > mostly_threshold)
            .unwrap_or(false)
    }
    ...
    pub fn last_vote_tx_blockhash_slot(&self) -> Slot {
        match self.last_vote_tx_blockhash {
            BlockhashStatus::Blockhash(_) => self.vote_state.last_voted_slot().unwrap_or(0),
            _ => 0,
        }
    }
    ...
    pub fn check_vote_stake_thresholds(
        ...
        let mut vote_state = self.vote_state.clone();
        vote_state.process_next_vote_slot(slot, pop_expired);
    ...
    pub fn is_slot_mostly_confirmed(
        ...
        if let Some(threshold) = self.mostly_confirmed_threshold {
            if let Some(&stake) = voted_stakes.get(&slot) {
                let lockout = stake as f64 / total_stake as f64;
...
pub mod test {
    ...
    fn test_check_vote_threshold_without_votes() {
        ...
        assert!(tower.check_vote_stake_thresholds(0, &stakes, 2, true).is_empty());
    }
    ...
    fn test_check_vote_threshold_no_skip_lockout_with_new_root() {
        ...
        assert!(!tower
            .check_vote_stake_thresholds(MAX_LOCKOUT_HISTORY as u64 + 1, &stakes, 2, true)
            .is_empty());
    }
    ...
    fn test_check_vote_threshold_below_threshold() {
        ...
        let stakes = vec![(0, 1)].into_iter().collect();
        tower.record_vote(0, Hash::default());
        assert!(!tower.check_vote_stake_thresholds(1, &stakes, 2, true).is_empty());
    }
    #[test]
    ...
    fn test_check_vote_thresholds_above_thresholds() {
        ...
        assert!(tower
            .check_vote_stake_thresholds(VOTE_THRESHOLD_DEPTH.try_into().unwrap(), &stakes, 4, true)
            .is_empty());
    }
    ...
    fn test_check_vote_threshold_deep_below_threshold() {
        ...
        assert!(!tower
            .check_vote_stake_thresholds(VOTE_THRESHOLD_DEPTH.try_into().unwrap(), &stakes, 10, true)
            .is_empty());
    }
    ...
    fn test_check_vote_threshold_above_threshold_after_pop() {
        ...
        assert!(tower.check_vote_stake_thresholds(6, &stakes, 2, true).is_empty());
    }
    ...
    fn test_check_vote_threshold_forks() {
        ...
        assert!(tower
            .check_vote_stake_thresholds(vote_to_evaluate, &voted_stakes, total_stake, true)
            .is_empty());
